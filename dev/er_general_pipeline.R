...
# --- Robust KMeans sweep: handles duplicate/identical rows safely
.er_kmeans_sweep <- function(X_dense, k_grid, seed=123, nstart=10) {
  ...
  # Collapse duplicates (near-duplicates rounded) to avoid "more centers than distinct points"
  key <- apply(round(Z, 8), 1, function(r) paste0(r, collapse="|"))
  uniq_keys <- unique(key)
  idx_u <- match(key, uniq_keys)           # map each row -> unique row index
  Zu <- Z[!duplicated(key), , drop = FALSE]
  n_u <- nrow(Zu)

  # Valid k: at least 2 and no more than number of unique rows
  k_use <- sort(unique(k_grid[k_grid >= 2 & k_grid <= max(1L, n_u - 0L)]))
  if (length(k_use) == 0 || n_u < 2) {
    return(list(k = 1L, cl = rep(1L, nrow(Z)), score = NA_real_))
  }
  ...
  for (k in k_use) {
    set.seed(seed)
    # cluster on unique rows only
    clu_u <- stats::kmeans(Zu, centers = k, nstart = nstart)$cluster
    # map back to all rows (duplicates inherit the unique row's label)
    cl <- clu_u[idx_u]

    # Recompute centers on full Z to score fairly
    ...
    cos_next <- vapply(seq_len(nrow(Z)), function(i) {
      if (k <= 1) return(0)
      max(cos_to_all[i, setdiff(seq_len(k), cl[i])])
    }, numeric(1))
    ...
    if (is.finite(s_mean) && s_mean > best$score) {
      best <- list(k = k, cl = as.integer(cl), score = s_mean)
    }
  }
  best
}

# --- Single KMeans run with duplicate guard (used elsewhere)
er_kmeans_from_X <- function(X, k=10, seed=123){
  set.seed(seed)
  rn <- sqrt(rowSums(X^2)); rn[rn==0] <- 1
  Z <- X / rn
  key <- apply(round(Z, 8), 1, function(r) paste0(r, collapse="|"))
  uniq_keys <- unique(key)
  idx_u <- match(key, uniq_keys)
  Zu <- Z[!duplicated(key), , drop = FALSE]
  n_u <- nrow(Zu)

  if (n_u < 2) return(rep(1L, nrow(Z)))
  k <- max(2L, min(k, n_u))               # cap k by distinct rows
  clu_u <- stats::kmeans(Zu, centers = k, nstart = 20)$cluster
  as.integer(clu_u[idx_u])
}
...
# 1) Robust single KMeans run (caps k by #distinct rows)
er_kmeans_from_X <- function(X, k=10, seed=123){
  set.seed(seed)
  # L2-normalize
  ...
  # collapse duplicates on rounded Z
  key <- apply(round(Z, 8), 1L, function(r) paste0(r, collapse="|"))
  uniq <- !duplicated(key)
  Zu <- Z[uniq, , drop = FALSE]
  idx_u <- match(key, unique(key))
  n_u <- nrow(Zu)

  if (n_u < 2L) return(rep(1L, nrow(Z)))  # all identical

  k_eff <- max(2L, min(as.integer(k), n_u))
  clu_u <- stats::kmeans(Zu, centers = k_eff, nstart = 20)$cluster
  as.integer(clu_u[idx_u])
}

# 2) Robust KMeans sweep (handles scalars, caps k, de-duplicates)
.er_kmeans_sweep <- function(X_dense, k_grid, seed=123, nstart=10) {
  # allow scalar k_grid
  k_grid <- as.integer(k_grid)
  if (length(k_grid) == 1L) k_grid <- 2:max(2L, k_grid)  # if user passed 1000, make 2:1000; if <=2, minimal
  ...
  if (n_u < 2L) {
    message("[KMeans] All rows identical after SVD; returning 1 cluster.")
    return(list(k = 1L, cl = rep(1L, nrow(Z)), score = NA_real_))
  }

  # valid k in [2, n_u]
  k_use <- sort(unique(k_grid[k_grid >= 2L & k_grid <= n_u]))
  if (!length(k_use)) {
    k_use <- 2:min(10L, n_u)  # small fallback sweep
  }
  message(sprintf("[KMeans] %d unique rows; capping k at %d. Evaluating %d k's.",
                  n_u, n_u, length(k_use)))
  ...
  for (k in k_use) {
    clu_u <- stats::kmeans(Zu, centers = k, nstart = nstart)$cluster
    cl <- as.integer(clu_u[idx_u])

    # centers in full Z
    centers <- rowsum(Z, cl) / as.vector(table(cl))
    cn <- sqrt(rowSums(centers^2)); cn[cn == 0] <- 1; centers <- centers / cn

    cos_self <- rowSums(Z * centers[cl, , drop = FALSE])
    cos_all  <- Z %*% t(centers)
    cos_next <- vapply(seq_len(nrow(Z)), function(i){
      if (k <= 1L) return(0)
      max(cos_all[i, setdiff(seq_len(k), cl[i])])
    }, numeric(1))
    s <- (cos_self - cos_next) / pmax(cos_self, cos_next, 1e-8)
    s_mean <- mean(s, na.rm = TRUE)
...
# 3) (Optional but recommended) guard inside er_compare_methods_unsupervised after SVD
#    Insert RIGHT AFTER you compute Xsvd (and Dcos if present):
#    - it won’t change outputs, just informs you about unique count during unsupervised runs
.er__announce_unique_rows <- function(Xsvd){
  nr <- sqrt(rowSums(Xsvd^2)); nr[nr==0] <- 1
  Z <- Xsvd / nr
  key <- apply(round(Z, 8), 1L, function(r) paste0(r, collapse="|"))
  n_u <- sum(!duplicated(key))
  message(sprintf("[Info] SVD rows: %d; distinct (rounded) rows: %d.", nrow(Z), n_u))
}
# Call once in er_compare_methods_unsupervised() right after you set Xsvd:
#   .er__announce_unique_rows(Xsvd)
...
# ------------------------- Facade (simple command) — with er_method alias
er_main <- function(data,
                    ...
                    knn_k = 15,
                    louvain_min_sim = 0.0,
                    # original vector-style switch (kept for backward compatibility)
                    use_methods = c("kmeans_tfidf_svd","louvain_knn","louvain_multifield"),
                    # NEW: simple alias; e.g. "kmeans" to run only KMeans
                    er_method = c("all"),
                    ...
) {
  # normalize original param
  use_methods <- match.arg(use_methods,
                           c("kmeans_tfidf_svd","louvain_knn","louvain_multifield"),
                           several.ok = TRUE)

  # --- NEW: normalize er_method -> use_methods mapping
  er_method <- tolower(er_method)
  er_map <- c(
    kmeans             = "kmeans_tfidf_svd",
    louvain            = "louvain_knn",
    multifield         = "louvain_multifield",
    louvain_knn        = "louvain_knn",
    louvain_multifield = "louvain_multifield",
    all                = "ALL"
  )
  if (length(er_method) && !all(er_method == "all")) {
    mapped <- unique(na.omit(unname(er_map[match(er_method, names(er_map))])))
    if (!length(mapped)) stop("er_method has no valid values. Use one of: ",
                              paste(setdiff(names(er_map),"all"), collapse=", "))
    use_methods <- intersect(use_methods, mapped)
    if (!length(use_methods)) stop("No methods left after er_method filtering.")
  }

  # Load data (same as your file)
  ...
  if ("kmeans_tfidf_svd" %in% use_methods) {
    km_best <- .er_kmeans_sweep(Xsvd, k_grid)  # same helper used in your file
  }

  # Path 2: Louvain on kNN graph
  lv_knn <- NULL
  ...
  # Path 3: optional multi-field Louvain
  lv_multi <- NULL
  ...
  out <- list(
    clusters = if (!is.null(km_best)) km_best$cl else if (!is.null(lv_knn)) lv_knn else if (!is.null(lv_multi)) lv_multi else rep(1L, n),
    k        = if (!is.null(km_best)) km_best$k else NA_integer_,
    chosen   = if (!is.null(km_best)) "kmeans_tfidf_svd" else if (!is.null(lv_knn)) "louvain_knn" else if (!is.null(lv_multi)) "louvain_multifield" else "none",
    summary  = list(fields = fields)
  )

  if (!is.null(write_csv)) {
    pred_df <- data.frame(id = df_text$id, cluster = out$clusters, stringsAsFactors = FALSE)
    try(readr::write_csv(pred_df, write_csv), silent = TRUE)
  }
  out
}
...
# ============================
# Unsupervised model selection — now with er_method/use_methods filter
# ============================
er_compare_methods_unsupervised <- function(
    ...
    er_method = c("all"),   # e.g. "kmeans" to run ONLY KMeans
    use_methods = NULL,     # tolerated for back-compat; ignored if er_method != "all"
    # plumbing
  ...
  # --- normalize requested methods (aliases) ---
  alias_map <- c(
    kmeans             = "kmeans",
    kmeans_tfidf_svd   = "kmeans",
    louvain            = "louvain_knn",
    louvain_knn        = "louvain_knn",
    multifield         = "louvain_multifield",
    louvain_multifield = "louvain_multifield",
  ...
  norm <- function(x){
    x <- tolower(x); unique(na.omit(unname(alias_map[match(x, names(alias_map))])))
  }

  requested <- norm(er_method)
  if (length(requested) == 0) requested <- "all"
  if (identical(requested, "all") && !is.null(use_methods)) {
    requested <- norm(use_methods); if (length(requested) == 0) requested <- "all"
  }

  run_all    <- identical(requested, "all")
  run_kmeans <- run_all || "kmeans" %in% requested
  # (we wire others only if you want them later)
  run_louvain <- run_all || "louvain_knn" %in% requested
  run_multi   <- run_all || "louvain_multifield" %in% requested
  run_embed   <- run_all || "embed_knn" %in% requested
  run_gc      <- run_all || "gc" %in% requested
  run_pam     <- run_all || "pam" %in% requested
  run_hc      <- run_all || "hc" %in% requested
  run_mstsn   <- run_all || "mstsn" %in% requested

  # -------- 1) load & features (reuses your existing utilities) --------
  p <- if (isTRUE(show_progress)) er_progress_start(5, "Unsupervised model selection") else NULL
  df_raw  <- er_load_input(data, sheet = sheet);                                                     er_progress_tick(p, "Loaded input")
  ...
  Dcos    <- er_cosine_dist(Xsvd);                                                                    er_progress_tick(p, "Cosine distances")

  best_rows <- list(); curves <- list()

  # -------- 2) tune per method (only KMeans if you asked) --------------
  if (run_kmeans) {
    # prefer your in-file fast sweep; fall back if absent
    if (exists(".er_kmeans_sweep", mode="function")) {
      km_best <- .er_kmeans_sweep(Xsvd, k_grid = k_grid)
      curves$kmeans <- tibble::tibble(method="kmeans", k=km_best$k, silhouette=km_best$score)
      best_rows$kmeans <- tibble::tibble(method="kmeans", k_clusters=km_best$k, silhouette=km_best$score)
    } else {
      # simple fallback
      rows <- lapply(k_grid, function(k){
        cl <- er_kmeans_from_X(Xsvd, k=k)
        tibble::tibble(method="kmeans", k_clusters=k, silhouette=er_silhouette_avg(cl, Dcos))
      })
      km_curve <- dplyr::bind_rows(rows)
      curves$kmeans <- km_curve
      best_rows$kmeans <- km_curve[which.max(km_curve$silhouette %||% NA_real_), , drop=FALSE]
    }
  }

  # (The following blocks only run if requested; harmless to leave for future)
  if (run_louvain && exists("er_louvain_knn", mode="function")) {
    rows <- lapply(knn_grid, function(k){
      labs <- er_louvain_knn(Xsvd, knn=k, min_sim=min(min_sim_grid))$labels
      tibble::tibble(method="louvain_knn", k_clusters=k, modularity=NA_real_, silhouette=er_silhouette_avg(labs, Dcos))
    })
    lv_curve <- dplyr::bind_rows(rows)
    curves$louvain_knn <- lv_curve
    best_rows$louvain_knn <- lv_curve[which.max(lv_curve$silhouette %||% NA_real_), , drop=FALSE]
  }

  if (run_multi && exists("er_similarity_multifield", mode="function") && exists("er_louvain_from_S", mode="function")) {
    spec <- list()
    if ("title"   %in% names(df_raw)) spec <- append(spec, list(list(name="title",   type="jw",  w=1)))
    if ("authors" %in% names(df_raw)) spec <- append(spec, list(list(name="authors", type="jw",  w=1)))
    if ("address" %in% names(df_raw)) spec <- append(spec, list(list(name="address", type="jw",  w=1)))
    if ("year"    %in% names(df_raw)) spec <- append(spec, list(list(name="year",    type="year", w=1, tau=0.5)))
    blk <- if ("title" %in% names(df_raw)) tolower(substr(df_raw$title %||% "", 1, 1)) else rep("ALL", nrow(df_raw))
    S <- er_similarity_multifield(df_raw, spec = spec, block_key = blk, top_k = max(knn_grid))
    labs <- er_louvain_from_S(S, min_sim = min(min_sim_grid))
    curves$louvain_multifield <- tibble::tibble(method="louvain_multifield",
                                                k_clusters=length(unique(labs)),
                                                silhouette=er_silhouette_avg(labs, Dcos))
    best_rows$louvain_multifield <- curves$louvain_multifield[1,]
  }

  if (run_embed && exists("er_safe_parse_embedding_col", mode="function") && "embedding" %in% tolower(names(df_raw))) {
    E <- er_safe_parse_embedding_col(df_raw[[grep("embedding", names(df_raw), ignore.case=TRUE, value=TRUE)[1]]])
    rows <- lapply(embed_k_grid, function(k){
      labs <- er_embed_knn(E, k=k, cos_thresh=max(embed_cos_grid))
      tibble::tibble(method="embed_knn", k_clusters=k, silhouette=er_silhouette_avg(labs, Dcos))
    })
    em_curve <- dplyr::bind_rows(rows)
    curves$embed_knn <- em_curve
    best_rows$embed_knn <- em_curve[which.max(em_curve$silhouette %||% NA_real_), , drop=FALSE]
  }

  if (run_gc && exists("er_gc_from_text", mode="function")) {
    gc <- er_gc_from_text(df_text$text_for_matching, thresholds = gc_thresholds, dist_method="jw", progress = p)
    curves$gc <- gc$tuning
    best_rows$gc <- tibble::tibble(method="gc", threshold = gc$best_threshold,
                                   silhouette = if ("silhouette" %in% names(gc$tuning)) gc$tuning$silhouette[gc$tuning$threshold==gc$best_threshold] else NA_real_)
  }

  if (inherits(p, "er_progress")) { er_progress_tick(p, "Compiled curves"); er_progress_done(p) }

  # -------- 3) stack & pick winner by objective --------
  avail <- Filter(Negate(is.null), best_rows)
  if (!length(avail)) stop("No methods were run (check er_method/use_methods).")
  leader <- dplyr::bind_rows(lapply(names(avail), function(nm){
    row <- avail[[nm]]
    if (!"method" %in% names(row)) row$method <- nm
    row
  }))

  # choose by objective
  pick <- switch(objective,
                 "silhouette" = leader[which.max(leader$silhouette %||% NA_real_), , drop=FALSE],
                 "modularity" = if ("modularity" %in% names(leader)) leader[which.max(leader$modularity %||% NA_real_), , drop=FALSE] else leader[1,,drop=FALSE],
                 "ch"         = if ("ch"         %in% names(leader)) leader[which.max(leader$ch         %||% NA_real_), , drop=FALSE] else leader[1,,drop=FALSE],
                 "db"         = if ("db"         %in% names(leader)) leader[which.min(leader$db         %||% Inf    ), , drop=FALSE] else leader[1,,drop=FALSE],
                 "ranked_mix" = {
                   # simple mix if you later populate more metrics
                   s <- scales::rescale(leader$silhouette %||% NA_real_, to=c(0,1), from=range(leader$silhouette, finite=TRUE))
                   leader[order(s, decreasing=TRUE)[1], , drop=FALSE]
                 }
  )
