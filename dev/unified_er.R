# =====================================================================
# ERBOT — Unified ER Pipeline (one-in-all, ready-to-run)
# Version: 2025-09-25
# Author: ChatGPT (ERBOT helper)
#
# This single script consolidates and reconciles your three editions:
#   1) er_pipeline_fullcode.txt
#   2) er_pipeline_weigthedfields_fullcode.txt
#   3) er_general_pipeline_fullcode.txt   (facade renamed to `er_main_simple`)
#
# Key choices:
# - We KEEP `er_main()` from er_pipeline_fullcode as the primary entry point.
# - We RENAME the simpler facade in the general file to `er_main_simple()`.
# - We KEEP the more robust `er_save_report_pdf()` implementation (from general),
#   which overwrites the earlier variant by virtue of appearing later in this file.
# - Weighted-fields pipeline (`er_general_pipeline`) is included as-is.
#
# How to use (quick):
#   source("erbot_unified_pipeline.txt")
#   res <- er_main(data = "cora", truth = cora_gold,
#                  fields = c("title","authors","address"),
#                  k_grid = seq(10, 300, by=10),
#                  write_csv = "cora_clean_pred.csv")
#   er_save_report_pdf(res, file = "cora_report.pdf", dataset_name = "CORA", top_n = 5)
#
#   # Simpler path explorer (no giant options):
#   res2 <- er_main_simple(data = "cora", truth = cora_gold,
#                          fields = c("title","authors","address"))
#
# Dependencies:
# - R packages: data.table, dplyr, stringr, stringdist, Matrix, irlba, igraph,
#               text2vec, ggplot2, grid, gridExtra, tibble, readr, stats, utils
# - Optional: GCMER (for clustering_agreement and resolve_entities), or source('rcode.R')
# =====================================================================



# ==== Begin: er_pipeline_fullcode.txt ====
# ==============================================================================
...
# ==== Begin: er_pipeline_weigthedfields_fullcode.txt ====
# =====================================================================
...
# ==== Begin: er_general_pipeline_fullcode.txt (er_main -> er_main_simple) ====
# ==============================================================================
# Unified ER Pipeline (cora / affiliation / D10K / NC voters / etc.)
# Methods: KMeans, MST/SN+Edit, Louvain+kNN, Embed-kNN,
#          HC, PAM, GC(resolve_entities)
